...
describe('param', function () {
  ...
  it('正确解析 Number 参数', function () {
    ...
    expect(new $.NumberParam('a', 123).isOk).to.equal(true);
    expect(new $.NumberParam('a', 123).isError).to.equal(false);
    expect(new $.NumberParam('a', 'xxx').isOk).to.equal(false);
    expect(new $.NumberParam('a', 'xxx').isError).to.equal(true);
  ...
  it('正确解析 NumberArray 参数', function () {
    ...
    expect(new $.NumberArrayParam('a', '123,456').value).to.deep.equal([ 123, 456 ]);
    expect(new $.NumberArrayParam('a', [ 123, 456 ]).value).to.deep.equal([ 123, 456 ]);
    expect(new $.NumberArrayParam('a', [ 123, 456, '789' ]).value).to.deep.equal([ 123, 456, 789 ]);
    expect(new $.NumberArrayParam('a', 'xxx').isError).to.equal(true);
    expect(new $.NumberArrayParam('a', [ 'xxx' ]).isError).to.equal(true);
    expect(new $.NumberArrayParam('a', [ 123, 'xxx' ]).isError).to.equal(true);
  });
  ...
  it('正确解析 String 参数', function () {
    ...
    expect(new $.StringParam('a', new Error('test')).value).to.equal('Error: test');
    expect(new $.StringParam('a', /hello/).value).to.equal('/hello/');
    expect(new $.StringParam('a', {}).isError).to.equal(true);
  });

  it('正确解析 StringArray 参数', function () {
    expect(new $.StringArrayParam('a', 111).value).to.deep.equal([ '111' ]);
    expect(new $.StringArrayParam('a', [ 111 ]).value).to.deep.equal([ '111' ]);
    expect(new $.StringArrayParam('a', [ 111, 'hello' ]).value).to.deep.equal([ '111', 'hello' ]);
    expect(new $.StringArrayParam('a', [ 111, 'hello', {} ]).isError).to.equal(true);
  });
  ...
  it('正确解析 Boolean 参数', function () {
    ...
    expect(new $.BooleanParam('a', {}).isError).to.equal(true);
  });
  ...
  it('正确解析 BooleanArray 参数', function () {
    ...
    expect(new $.BooleanArrayParam('a', undefined).value).to.deep.equal([ false ]);
    expect(new $.BooleanArrayParam('a', [ true, false, 'on', 'off', 1, 0, null, undefined, 'yes', 'no' ]).value).to.deep.equal([ true, false, true, false, true, false, false, false, true, false ]);
    expect(new $.BooleanArrayParam('a', [ 1, 'xxx' ]).isError).to.equal(true);
  });
  ...
  it('正确解析 JSON 参数', function () {
    ...
    expect(new $.JSONParam('a', '{"a":123,"b":"456"}').value).to.deep.equal({ a: 123, b: '456' });
    expect(new $.JSONParam('a', '"a').isError).to.equal(true);
    expect(new $.JSONParam('a', '/').isError).to.equal(true);
    expect(new $.JSONParam('a', null).isError).to.equal(true);
    expect(new $.JSONParam('a', true).isError).to.equal(true);
    expect(new $.JSONParam('a', {}).isError).to.equal(true);
  });
  ...
  it('正确解析 Date 参数', function () {
    ...
    expect(new $.DateParam('a', 'xxx').isError).to.equal(true);
    expect(new $.DateParam('a', '12345678').isError).to.equal(true);
    expect(new $.DateParam('a', {}).isError).to.equal(true);
  });

  it('正确解析 DateArray 参数', function () {
    const date = new Date(1502000847000);
    expect(new $.DateArrayParam('a', date).value).to.deep.equal([ date ]);
    expect(new $.DateArrayParam('a', date.getTime()).value).to.deep.equal([ date ]);
    expect(new $.DateArrayParam('a', date.toString()).value).to.deep.equal([ date ]);
    expect(new $.DateArrayParam('a', [ date.getTime(), date.toString() ]).value).to.deep.equal([ date, date ]);
    expect(new $.DateArrayParam('a', [ date.getTime(), 'xxx' ]).isError).to.equal(true);
  });
  ...
  it('正确解析 Any 参数', function () {
    ...
    expect(new $.AnyParam('a', [ 'hello', 123 ]).value).to.deep.equal([ 'hello', 123 ]);
    expect(new $.AnyParam('a', { a: 111 }).value).to.deep.equal({ a: 111 });
    expect(new $.AnyParam('a', undefined).isError).to.equal(true);
  });
  ...
  it('Any 参数正确转换到其他参数', function () {
    ...
    expect(new $.AnyParam('a', 'on').toBooleanArray().value).to.deep.equal([ true ]);
    expect(new $.AnyParam('a', '111,222').toStringArray().value).to.deep.equal([ '111,222' ]);
  });
